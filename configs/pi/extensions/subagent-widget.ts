...
interface SubState {
  ...
  proc?: ReturnType<typeof spawn>; // active ChildProcess ref (for kill on /subrm)
  timerId?: ReturnType<typeof setInterval>; // elapsed timer ID (cleared on remove)
  removing?: boolean; // set before removal to suppress stale process output
}

export default function (pi: ExtensionAPI) {
  const agents: Map<number, SubState> = new Map();
  // Monotonic counter — never resets across sessions so IDs are always unique.
  let nextId = Number(process.env.PI_SUBAGENT_NEXT_ID ?? "1");
  ...
  function makeSessionFile(id: number): string {
    ...
    try {
      ...
      throw new Error(
        `Failed to create subagent session directory at ${dir}: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
      ...
      widgetCtx.ui.setWidget(key, (_tui: any, theme: any) => {
        ...
        return {
          render(width: number): string[] {
            ...
            if (lastLine) {
              const trimmed =
                lastLine.length > width - 10 ? lastLine.slice(0, width - 13) + "..." : lastLine;
    ...
    return new Promise<void>((resolve) => {
      const proc = spawn(
        ...
        [
          ...
          "-p",
          "--session",
          state.sessionFile, // persistent session for /subcont resumption
          "--no-extensions",
          "--model",
          model,
          "--tools",
  ...
  pi.registerTool({
    ...
    description:
      "Spawn a background subagent to perform a task. Returns the subagent ID immediately while it runs in the background. Results will be delivered as a follow-up message when finished.",
  ...
  pi.registerTool({
    ...
    execute: async (callId, args, _signal, _onUpdate, ctx) => {
      ...
      return {
        content: [
          { type: "text", text: `Subagent #${args.id} continuing conversation in background.` },
        ],
      };
    },
  });
  ...
  pi.registerTool({
    ...
    execute: async (callId, args, _signal, _onUpdate, ctx) => {
      ...
      // Mark as removing to suppress stale process output.
      state.removing = true;
  ...
  pi.registerCommand("subclear", {
    ...
    handler: async (_args, ctx) => {
      ...
      agents.clear();
      // Keep nextId monotonic — don't reset, so IDs stay unique across clears.
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    agents.clear();
    // Keep nextId monotonic — don't reset, so IDs stay unique across sessions.
  });
}
