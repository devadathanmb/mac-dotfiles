/**
 * Footer — Rich two-line custom status bar
 *
 * Line 1: provider/model + context meter + usage tokens on left, tokens in/out + cost on right
			...
			return {
				...
				render(width: number): string[] {
					...
					const l1Left =
						...
						theme.fg("dim", ` (${fmt(ctxTokens)}/${fmt(ctxWindow)})`);

					const sep = theme.fg("dim", " · ");
					const sep2 = theme.fg("dim", " | ");
					const l1Right =
						theme.fg("success", `${fmt(tokIn)}`) +
						theme.fg("dim", " in") + sep +
						theme.fg("accent", `${fmt(tokOut)}`) +
						theme.fg("dim", " out") + sep2 +
