    ...
    handler: async (args, ctx) => {
      ...
      const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
        ...
        const doGenerate = async () => {
          ...
          // Add a 60s timeout on top of the loader's abort signal.
          const timeoutController = new AbortController();
          const timeoutId = setTimeout(() => {
            timeoutController.abort(new Error("Handoff prompt generation timed out after 60s."));
          }, 60_000);

          // Combine loader signal with timeout: whichever fires first wins.
          const combinedSignal = AbortSignal.any([loader.signal, timeoutController.signal]);
